...
void video_renderer_init(SDL_Window* window ,Settings* settings, int width, int height) {
    ...
    for (int i = 0; i < settings->filter_types_array.count; i++) {
        ...
        case FILTER_TYPE_GLITCH:
            ...
            break;
        case FILTER_TYPE_CHROMA:
            filters_shaders[i] = shader_compile(filter_vertex_src, chromatic_src, NULL);
            break;
        ...
        case FILTER_TYPE_EVIL:
            ...
            break;
        case FILTER_TYPE_FISH:
            filters_shaders[i] = shader_compile(filter_vertex_src, fish_src, NULL);
            break;
        }
    }
